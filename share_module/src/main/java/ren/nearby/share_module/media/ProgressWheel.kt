...
package ren.nearby.share_module.media
...
class ProgressWheel : View {
    ...
    constructor(context: Context, attrs: AttributeSet) : super(context, attrs) {
        ...
        parseAttributes(context.obtainStyledAttributes(attrs, R.styleable.Share_ProgressWheel))
    ...
    private fun setupBounds(layout_width: Int, layout_height: Int) {
        ...
        if (!fillRadius) {
            ...
            circleBounds = RectF(
                (xOffset + barWidth).toFloat(), (yOffset + barWidth).toFloat(), (xOffset + circleDiameter - barWidth).toFloat(),
    ...
    private fun parseAttributes(a: TypedArray) {
        ...
        circleRadius =
            TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, circleRadius.toFloat(), metrics).toInt()
        ...
        barSpinCycleTime =
            a.getInt(R.styleable.ProgressWheel_matProg_barSpinCycleTime, barSpinCycleTime.toInt()).toDouble()
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (isSpinning) {
            ...
            if (!linearProgress) {
                val factor = 2.0f
                offset =
                    (1.0f - Math.pow((1.0f - mProgress / 360.0f).toDouble(), (2.0f * factor).toDouble())).toFloat() * 360.0f
                progress =
                    (1.0f - Math.pow((1.0f - mProgress / 360.0f).toDouble(), factor.toDouble())).toFloat() * 360.0f
            }
    ...
    override fun onSaveInstanceState(): Parcelable? {
        val v = super.onSaveInstanceState()
        val ss = WheelSavedState(v!!)
